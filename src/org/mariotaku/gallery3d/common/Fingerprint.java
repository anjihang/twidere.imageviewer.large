...
public class Fingerprint {
	 ...
	 */
	public Fingerprint(final byte[] bytes) {
		if (bytes == null || bytes.length != FINGERPRINT_BYTE_LENGTH) throw new IllegalArgumentException();
		mMd5Digest = bytes;
	}

	public boolean equals(final byte[] md5Digest) {
	...
	public String toStreamId() {
		final StringBuilder streamId = new StringBuilder(STREAM_ID_CS_PREFIX);
	 ...
	 */
	public static Fingerprint extractFingerprint(final List<String> streamIdList) {
		for (final String streamId : streamIdList) {
			if (streamId.startsWith(STREAM_ID_CS_PREFIX)) return fromStreamId(streamId);
		}
		return null;
	}
	 ...
	 * 
	 * @param byteCount length of original data will be stored at byteCount[0]
	 *            as a side product of the fingerprint calculation
	 */
	public static Fingerprint fromInputStream(final InputStream stream, final long[] byteCount) throws IOException {
	 ...
	 */
	public static Fingerprint fromStreamId(final String streamId) {
		...
		final byte[] bytes = new byte[FINGERPRINT_BYTE_LENGTH];
		int byteIdx = 0;
		for (int idx = STREAM_ID_CS_PREFIX.length(); idx < STREAM_ID_CS_01_LENGTH; idx += 2) {
			final int value = toDigit(streamId, idx) << 4 | toDigit(streamId, idx + 1);
	...
	private static void appendHexFingerprint(final StringBuilder sb, final byte[] bytes) {
		for (int idx = 0; idx < FINGERPRINT_BYTE_LENGTH; idx++) {
			final int value = bytes[idx];
			sb.append(Integer.toHexString(value >> 4 & 0x0f));
			sb.append(Integer.toHexString(value & 0x0f));
		}
	}

	private static int toDigit(final String streamId, final int index) {
		final int digit = Character.digit(streamId.charAt(index), 16);
		if (digit < 0) throw new IllegalArgumentException("illegal hex digit in " + streamId);
		return digit;
	}
}
