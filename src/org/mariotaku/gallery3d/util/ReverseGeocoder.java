...
public class ReverseGeocoder {
	...
	public ReverseGeocoder(final Context context) {
		mContext = context;
		mGeocoder = new Geocoder(mContext);
		mGeoCache = CacheManager.getCache(context, GEO_CACHE_FILE, GEO_CACHE_MAX_ENTRIES, GEO_CACHE_MAX_BYTES,
				GEO_CACHE_VERSION);
		mConnectivityManager = (ConnectivityManager) context.getSystemService(Context.CONNECTIVITY_SERVICE);
	}

	public String computeAddress(final SetLatLong set) {
		...
		if (currentCity.equals(addr1Locality) || currentCity.equals(addr2Locality)) {
			...
			if (closestCommonLocation != null && !"null".equals(closestCommonLocation)) return closestCommonLocation;
		}
		...
		if (closestCommonLocation != null && !"".equals(closestCommonLocation)) {
			final String adminArea = addr1AdminArea;
			final String countryCode = addr1CountryCode;
		...
		if (distance < MAX_LOCALITY_MILE_RANGE) {
			...
			if (closestCommonLocation != null) return closestCommonLocation;
		...
		if (closestCommonLocation != null && !"".equals(closestCommonLocation)) return closestCommonLocation;
	...
	public Address lookupAddress(final double latitude, final double longitude, final boolean useCache) {
		try {
			final long locationKey = (long) (((latitude + LAT_MAX) * 2 * LAT_MAX + (longitude + LON_MAX)) * EARTH_RADIUS_METERS);
			...
			if (cachedLocation == null || cachedLocation.length == 0) {
				...
				final List<Address> addresses = mGeocoder.getFromLocation(latitude, longitude, 1);
				if (!addresses.isEmpty()) {
					address = addresses.get(0);
					final ByteArrayOutputStream bos = new ByteArrayOutputStream();
					final DataOutputStream dos = new DataOutputStream(bos);
					final Locale locale = address.getLocale();
					...
					final int numAddressLines = address.getMaxAddressLineIndex();
				...
				final DataInputStream dis = new DataInputStream(new ByteArrayInputStream(cachedLocation));
				final String language = readUTF(dis);
				final String country = readUTF(dis);
				final String variant = readUTF(dis);
				Locale locale = null;
				if (language != null) {
					if (country == null) {
				...
				final int numAddressLines = dis.readInt();
				for (int i = 0; i < numAddressLines; ++i) {
	...
	private String checkNull(final String locality) {
	...
	private String getLocalityAdminForAddress(final Address addr, final boolean approxLocation) {
		...
		if (localityAdminStr != null && !"null".equals(localityAdminStr)) {
			...
			final String adminArea = addr.getAdminArea();
			if (adminArea != null && adminArea.length() > 0) {
	...
	private String valueIfEqual(final String a, final String b) {
		return a != null && b != null && a.equalsIgnoreCase(b) ? a : null;
	}

	public static final String readUTF(final DataInputStream dis) throws IOException {
		final String retVal = dis.readUTF();
		if (retVal.length() == 0) return null;
		return retVal;
	}

	public static final void writeUTF(final DataOutputStream dos, final String string) throws IOException {
		if (string == null) {
			dos.writeUTF("");
		} else {
			dos.writeUTF(string);
